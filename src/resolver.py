"""LLM context resolver (assignment Day 3).

For each section chunk, send breadcrumb + surrounding context + chunk text to an
LLM whose only job is to resolve cross-references and tag defined terms.
Emits enriched JSON for a downstream extraction LLM.
...
def build_section_catalog(chunks: list[dict]) -> list[dict]:
    ...
    Excerpts stay short on purpose — full text is filled in after the LLM picks
    a target chunk_id / heading.
    """
    catalog: list[dict] = []
...
def _fill_excerpts(
    ...
    *,
    ...
    catalog_by_heading: dict[str, dict],
) -> None:
    ...
    for r in refs:
        ...
        if target is None and r.get("target_heading"):
            target = catalog_by_heading.get(_clip(r["target_heading"], 200).lower())
            # catalog_by_heading maps lower heading → chunk
        if target is None and r.get("target_heading"):
            want = r["target_heading"].strip().lower()
            for ch in chunks_by_id.values():
                if (ch.get("heading") or "").strip().lower() == want:
                    target = ch
                    break
        if target:
...
def resolve_chunk_with_llm(
    ...
    *,
    ...
    catalog_by_heading = {
        (e.get("heading") or "").strip().lower(): chunks_by_id.get(e["chunk_id"], e)
        for e in catalog
    }
    _fill_excerpts(
        ...
        catalog_by_heading=catalog_by_heading,
    )
